...
function WarRoomContent() {
  ...
  if (!numericId) {
    return <div className="card p-6 font-mono text-sm">No customer ID provided.</div>;
  }
  ...
  return (
    <div className="space-y-8">
      ...
      {streamError && (
        <div className="card p-4 font-mono text-sm" style={{ color: "var(--risk)", borderColor: "var(--risk)" }}>
          {streamError}
        </div>
      )}

      <div className="space-y-4">
        <AgentCard index={1} title="Risk Analyst" subtitle="XGBoost scoring · SHAP attribution" accent="var(--risk)" status={a1Status}>
          {a1Data && (
            <div className="space-y-4">
              <p className="text-sm leading-relaxed" style={{ color: "var(--text-dim)" }}>{a1Data.risk_summary}</p>
              {a1Data.shap_drivers.length > 0 && (
                <div>
                  <div className="font-mono text-[11px] uppercase tracking-wider mb-3" style={{ color: "var(--text-faint)" }}>Agent SHAP Drivers</div>
        ...
        <AgentCard index={2} title="Evidence Collector" subtitle="RAG search · similar profiles · churn reasons" accent="var(--warn)" status={a2Status}>
          {a2Data && (
            <div className="space-y-4">
              <p className="text-sm leading-relaxed whitespace-pre-wrap" style={{ color: "var(--text-dim)" }}>{a2Data.evidence_report}</p>
        ...
        <AgentCard index={3} title="Retention Strategist" subtitle="Offer generation · policy · CRM logging" accent="var(--safe)" status={a3Status}>
          {a3Data && (
            <div className="space-y-4">
              <div className="rounded-lg p-4 text-sm leading-relaxed" style={{ background: "var(--safe-dim)", color: "var(--text)" }}>
                {a3Data.retention_offer}
              </div>
              <div className="flex items-center gap-3 font-mono text-[11px]" style={{ color: "var(--text-faint)" }}>
                <span className="flex items-center gap-1" style={{ color: a3Data.crm_logged ? "var(--safe)" : "var(--risk)" }}>
                  <span className="inline-block w-1.5 h-1.5 rounded-full" style={{ background: a3Data.crm_logged ? "var(--safe)" : "var(--risk)" }} />
                  {a3Data.crm_logged ? "CRM logged" : "CRM log failed"}
                </span>
                {a3Data.crm_log_id && <span>Log ID: {a3Data.crm_log_id.slice(0, 8)}</span>}
              </div>
            </div>
          )}
        </AgentCard>
...
export default function CustomerPage() {
  return (
    <Suspense fallback={<div className="card h-32 shimmer" />}>
      <WarRoomContent />
    </Suspense>
  );
}
...
function BackLink() {
  return (
    <Link href="/" className="inline-flex items-center gap-1.5 font-mono text-[13px] no-underline transition-colors" style={{ color: "var(--text-faint)" }}>
...
function DetailStat({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <div className="mt-0.5 font-mono text-sm tnum" style={{ color: "var(--text-dim)" }}>{value}</div>
    </div>
  );
}